...
if exists("python_highlight_all") && python_highlight_all != 0
  ...
  if !exists("python_highlight_regex")
    let python_highlight_regex = 1
  endif
endif
...
" Strings
syn region pythonString		start=+[bB]\='+ skip=+\\\\\|\\'\|\\$+ excludenl end=+'+ end=+$+ keepend contains=pythonEscape,pythonEscapeError
syn region pythonString		start=+[bB]\="+ skip=+\\\\\|\\"\|\\$+ excludenl end=+"+ end=+$+ keepend contains=pythonEscape,pythonEscapeError
syn region pythonString		start=+[bB]\="""+ end=+"""+ keepend contains=pythonEscape,pythonEscapeError,pythonDocTest2,pythonSpaceError
syn region pythonString		start=+[bB]\='''+ end=+'''+ keepend contains=pythonEscape,pythonEscapeError,pythonDocTest,pythonSpaceError
...
" Spell check to Gettext strings
syn region pythonGettext	start=+_([uUrR]\?'+ skip=+\\\\\|\\'\|\\$+ excludenl end=+'+ end=+$+ keepend contains=@Spell
syn match pythonGettextString "[^_(]*" contained containedin=pythonGettext

if exists("python_highlight_regex") && python_highlight_regex != 0
  " Regex highlight
  syn region pythonRegex	start=+re.[A-Za-z]*(.\?'+ skip=+\\\\\|\\'\|\\$+ excludenl end=+'+ end=+$+ keepend contains=pythonRawEscape
  syn region pythonRegex	start=+re.[A-Za-z]*(.\?"+ skip=+\\\\\|\\"\|\\$+ excludenl end=+"+ end=+$+ keepend contains=pythonRawEscape
  syn region pythonRegex	start=+re.[A-Za-z]*(.\?"""+ skip=+\\\\\|\\"""\|\\$+ excludenl end=+"""+ end=+$+ keepend contains=pythonRawEscape
  syn region pythonRegex	start=+re.[A-Za-z]*(.\?'''+ skip=+\\\\\|\\'''\|\\$+ excludenl end=+'''+ end=+$+ keepend contains=pythonRawEscape

  syn match pythonRegexNormal "re.[a-z]*(" contained containedin=pythonRegex
  syn match pythonRegexQuantifiers "[*+?]\|{[0-9,]*}" contained containedin=pythonRegex,pythonRegexGroup,pythonRegexGroupContent
  syn match pythonRegexEscaped "\\." contained containedin=pythonRegex,pythonRegexList
  syn match pythonRegexGroup "([^)]*)\|)" contained containedin=pythonRegex
  syn match pythonRegexGroupContent "[^()]" contained containedin=pythonRegexGroup
  syn match pythonRegexGroupName "(?P<[^>]*>\|(?[=iLmsux:!<][=!]\?" contained containedin=pythonRegexGroup
  syn match pythonRegexComment "(?#[^)]*)" contained containedin=pythonRegexGroup
  syn match pythonRegexList "\[[^[]*\]" contained containedin=pythonRegex,pythonRegexGroup
endif
...
if version >= 508 || !exists("did_python_syn_inits")
  ...
  HiLink pythonGettext    	       Normal
  HiLink pythonGettextString       String

  HiLink pythonRegex    	       String
  HiLink pythonRegexNormal         Normal
  HiLink pythonRegexQuantifiers    Statement
  HiLink pythonRegexList           Special
  HiLink pythonRegexEscaped        Special
  HiLink pythonRegexGroup          Number
  HiLink pythonRegexGroupContent   String
  HiLink pythonRegexGroupName      pythonRegexGroup
  HiLink pythonRegexComment        Comment
